...
void logger_set_level(logger_t* logger, int level) { logger->level = level; }
...
static void logfile_name(const char* filepath, time_t ts, char* buf, int len) {
  ...
  snprintf(buf, len, "%s.%04d%02d%02d.log", filepath, tm->tm_year + 1900,
...
static FILE* logfile_shift(logger_t* logger) {
  ...
  // open today logfile
  if (logger->fp_ == NULL) {
    logfile_name(logger->filepath, ts_now, logger->cur_logfile,
                 sizeof(logger->cur_logfile));
  ...
  if (logger->fp_ && --logger->can_write_cnt < 0) {
    ...
    if (filesize > logger->max_filesize) {
      ...
      logger->can_write_cnt =
          (logger->max_filesize - filesize) / logger->bufsize;
    }
  }
...
int logger_print(logger_t* logger, int level, const char* fmt, ...) {
  ...
  // 这里XXX 和LOG_LEVEL_MAP组合做成了一个switch选择
